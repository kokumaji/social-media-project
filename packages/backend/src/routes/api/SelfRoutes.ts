...
export const handleRequest: RH = (server) => async (req: Request, res: Response) => {
    const token = req.headers.authorization?.split(' ')[1];

    if(token) {
        const payload: TokenPayload = jwt.verify(token, server.options.authSecret as string) as TokenPayload;
