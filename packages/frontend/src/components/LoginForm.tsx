...
class LoginFormWrapper extends React.Component<{ userState: UserStateProvider }, { username: string, password: string, loginFailed: boolean, showPwd: boolean }> {
    state = {
        username: "",
        password: "",
        loginFailed: false,
    ...
    private async tryLogin(e: React.MouseEvent<HTMLButtonElement, MouseEvent>) {
        e.preventDefault();
        try { 
            let successful = await this.props.userState.authenticate(this.state.username, this.state.password);
    ...
    render() {
        ...
        const pwdStyle = { borderStyle: `${borderStyle}`, borderColor: `${borderColor}`, color: `${borderColor}`, transition: '0.1s' };
        ...
        return !this.props.userState.authed ? (
            <LoginContainer>
                ...
                <Form.InputField maxLength={20} value={this.state.username} placeholder="Username" onChange={this.handleUsernameInput.bind(this)} />
                <Form.FieldRow>
                    <Form.PasswordField style={pwdStyle} maxLength={50} value={this.state.password} placeholder="Password"  type={fieldType} onChange={this.handlePasswordInput.bind(this)} />
...
export const LoginForm = () => <UserContext.Consumer>{(state) => <LoginFormWrapper userState={state} />}</UserContext.Consumer>
    // <LoginContainer>
    //     <h1>Social Media WIP</h1>
